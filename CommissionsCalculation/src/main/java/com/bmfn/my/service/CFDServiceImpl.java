...
public class CFDServiceImpl implements CFDService {
    ...
    public String getTotalCommissionsPerIB(int ibId, String firstCloseTime, String lastCloseTime) {
        ...
        try{
            ...
            Query query = session.createQuery("SELECT SUM(commission) FROM TradersCFD WHERE " +
                    "login in (SELECT login FROM UsersCFD WHERE group NOT LIKE '%MAM%' AND code like:ibId) " +
