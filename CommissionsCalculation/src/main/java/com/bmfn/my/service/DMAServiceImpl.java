...
public class DMAServiceImpl implements DMAService {
    ...
    public String getTotalCommissionsPerIB(int ibId, String firstCloseTime, String lastCloseTime) {
        ...
        try{
            ...
            Query query = session.createQuery("SELECT SUM(commission) FROM TradersDMA WHERE " +
                    "login in (SELECT login FROM UsersDMA WHERE group NOT LIKE '%MAM%' AND code like:ibId) " +
